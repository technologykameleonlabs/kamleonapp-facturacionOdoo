-- =============================================
-- SEED: VISTAS DE ANÁLISIS PARA EP-020 (VERSION SIMPLIFICADA)
-- EP-020 - Sistema de apuntes unificados
-- =============================================

-- Vista detallada básica (solo con tablas que existen)
...
SELECT
    ...
    -- Para esta versión simplificada, solo resolvemos proyectos básicos
    CASE
    ...
    CASE
        WHEN ae.reference_model = 'project' THEN 'Proyecto referenciado'
        ELSE 'Otro objeto'
    END as resolved_project_name,

    -- Validación básica
    CASE
        WHEN ae.reference_model IN ('task_time', 'project', 'department', 'employee') THEN true
        ELSE false
...
SELECT
    ...
    ac.name as category_name,
    as_stage.name as stage_name,
    DATE_PART('day', CURRENT_DATE - ae.entry_date) as days_pending
FROM analytics.trn_analytic_entries ae
JOIN analytics.mst_analytics_entry_types aet ON ae.entry_type_id = aet.id
LEFT JOIN analytics.mst_analytics_categories ac ON ae.category_id = ac.id
JOIN analytics.mst_analytics_stages as_stage ON ae.stage_id = as_stage.id
...
-- Mostrar ejemplo de cada vista
SELECT 'VISTA DETALLADA (primeros 3):' as example;
SELECT entry_date, entry_type_name, category_name, amount, stage_name
FROM analytics.v_analytic_entries_detailed
LIMIT 3;

SELECT 'VISTA POR TIPO:' as example;
SELECT * FROM analytics.v_entries_by_type;

SELECT 'VISTA BALANCE:' as example;
SELECT * FROM analytics.v_balance_summary;
