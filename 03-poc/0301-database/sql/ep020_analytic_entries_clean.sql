...
-- Insertar tipos basicos (idempotente)
...
('INDIRECT_COST', 'Coste Indirecto', 'Costes indirectos (reuniones, gestion)', 'cost', '#4ECDC4', 2),
('GENERAL_EXPENSE', 'Gasto General', 'Gastos generales de empresa/departamento', 'cost', '#45B7D1', 3),
('REVENUE', 'Ingreso', 'Ingresos por ventas/consultoria', 'revenue', '#96CEB4', 4),
...
-- Insertar categorias basicas (idempotente)
...
('CONS', 'Consultoria', 'revenue', '#54A0FF')
ON CONFLICT (code) DO NOTHING;
...
-- Insertar stages basicos (idempotente)
INSERT INTO analytics.mst_entry_stages (code, name, description, color, sort_order) VALUES
('DRAFT', 'Borrador', 'Apunte en elaboracion, no visible en reportes', '#6B7280', 1),
('PENDING_APPROVAL', 'Pendiente Aprobacion', 'Esperando aprobacion del workflow', '#F59E0B', 2),
...
-- =============================================
-- 2. TABLA PRINCIPAL DE APUNTES ANALITICOS
-- =============================================
...
-- Indices compuestos para consultas comunes
...
-- Funcion para obtener stage por defecto (Draft)
...
-- Funcion para validar referencias
...
BEGIN
    -- Validar segun el modelo de referencia
...
-- Funcion para conversion simple de moneda (EUR como base)
...
-- =============================================
-- 5. TRIGGER PARA CONVERSION AUTOMATICA
-- =============================================
...
-- =============================================
-- 6. VISTAS PARA ANALISIS
-- =============================================

-- Vista detallada con resolucion de referencias
...
SELECT
    ...
    -- Resolver proyecto dinamicamente
    CASE
    ...
    -- Resolver proyecto name dinamicamente
    CASE
        WHEN ae.reference_model = 'task_time' THEN
...
-- Crear trigger para stage por defecto
...
COMMENT ON TABLE analytics.mst_categories IS 'Maestro de categorias de apuntes';
